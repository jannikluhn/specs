...
- `getMember(i)` returns `getMembers()[i]`.
- `getThreshold()` returns a number `t` with `1 <= t <= n`.
- The return values of `isFinalized`, `getNumMembers`, `getMember`, `getMembers`, and `getThreshold` are guaranteed to not change in the future.
...
```Python
def encrypt(message: bytes, bytes_epoch_id: bytes, eon_key: G2, sigma: Block) -> EncryptedMessage:
    message_blocks = pad_and_split(message)
    r = compute_r(sigma)
    g1_epoch_id = compute_g1_epoch_id(bytes_epoch_id)
    return (
        compute_c1(r),
        compute_c2(sigma, r, g1_epoch_id, eon_key),
...
def compute_c2(sigma: Block, r: int, g1_epoch_id: G1, eon_key: G2) -> Block:
...
def compute_g1_epoch_id(bytes_epoch_id: bytes) -> G1:
    h = keccak256(bytes_epoch_id)
    i = int.from_bytes(h, "big")
